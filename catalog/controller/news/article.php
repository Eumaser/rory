...
class ControllerNewsArticle extends Controller {
	public function index() {
		...
		//$data['edr_image_a'] = $news_info['image'];
		//$data['edr_image_b'] = $news_info['image2'];
	 // echo '<pre>';
    ...
    $bheight = ($this->config->get('ncategory_bnews_thumb_height')) ? $this->config->get('ncategory_bnews_thumb_height') : 230;
		...
		if ($news_info['image']) {
			...
			//image/catalog/rory/SLICINGS/article/article-img.jpg
				$data['thumb'] = '';
				//	$data['thumb'] = 'image/catalog/rory/SLICINGS/article/article-img.jpg';
				$data['popup'] = '';
		}
		...
		//	die();
			...
			//echo '<pre>';
			//print_r($data);die();
			//return $this->load->view($this->config->get('config_template') . '/template/news/article.php', $data);
		} else {
