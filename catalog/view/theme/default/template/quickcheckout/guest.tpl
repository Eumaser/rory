...
<?php foreach ($fields as $field) { ?>
  <?php if ($field == 'country') { ?>
	...
	<div class="guest-form col-sm-6<?php echo !empty(${'field_' . $field}['required']) ? ' required' : ''; ?>">
  ...
  <?php } else { ?>
	<?php if (!empty(${'field_' . $field}['display'])) { ?>
	<div <?php echo $field == 'postcode' ? ' id="payment-postcode-required"' : ''; ?> class="guest-form col-sm-6 form-group <?php echo !empty(${'field_' . $field}['required']) ? ' required' : ''; ?>">
	  <input type="text" placeholder="<?php echo ${'entry_' . $field}; ?>" name="<?php echo $field; ?>" value="<?php echo ${$field} ? ${$field} : ${'field_' . $field}['default']; ?>" class="form-control"  id="input-payment-<?php echo str_replace('_', '-', $field); ?>" />
	</div>
	<?php } else { ?>
